...
int main(int argc, char **argv) {
    ...
    obj_filename = argv[3];
    num_points = atof(argv[4]);

    cout << "Running Experiment: PRM 3D" << "Num points: " << num_points << endl;
    cout << "--------------------------------" << endl;


    pcd_name = pcd_filename.substr(pcd_filename.find_last_of("/\\") + 1);
    output_filename = "output/experiment_grid_" + num_points + pcd_name + ".json";
    ...
    skeleton_finder_3D.run_processing_random_3d(
        cloud_xyz, 
        num_points,
        config["base_radius"].as<double>(),
    );   
