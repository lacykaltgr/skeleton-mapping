...
export const loadMeshMap = (world, global) => {
	gltfLoader.load('assets/gltf/mesh.gltf', (gltf) => {
		const model = gltf.scene;
		model.traverse((node) => {
			if (node.material) {
				const reconstructedMaterial = new MeshStandardMaterial({
		...
		const meshMap = new Group().add(model.clone());
		meshMap.scale.set(0.25, 0.25, -0.25);
		meshMap.rotateX(-Math.PI / 2);
		global.meshMap = meshMap;
		global.meshMap.grabComponent = world
			.createEntity()
			.addComponent(GrabComponent, { object3D: meshMap });
	});
};


export const loadMap = (world, global) => {

	const loader = new PCDLoader();
	loader.load(
		'assets/pointcloud.pcd',
		function (points) {
			// rotate the point cloud so that it is oriented correctly
			points.rotateX(-Math.PI / 2);
			// scale the point cloud so that it is the correct size
			//points.scale(0.9, 0.9, 0.9);

			console.log("added point cloud");

			global.pcd = points;
			//global.pcd.grabComponent = world
			//	.createEntity()
			//	.addComponent(GrabComponent, { object3D: points });
		},
		function (xhr) {
