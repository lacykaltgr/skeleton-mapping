...
#include <iostream>
#include <limits>
#include "SkeletonFinder/quickhull/MathUtils.hpp"
#include "SkeletonFinder/quickhull/Structs/Mesh.hpp"
...
template <typename T>
ConvexHull<T> QuickHull<T>::getConvexHull(const Vector3<T>* vertexData, size_t vertexCount,
...
template <typename T>
void QuickHull<T>::buildMesh(const VertexDataSource<T>& pointCloud, bool CCW,
                             bool useOriginalIndices, T epsilon) {
  ...
  m_planar = false;  // The planar case happens when all the points appear to lie on a two
...
void QuickHull<T>::createConvexHalfEdgeMesh() {
  ...
  while (!m_faceList.empty()) {
    ...
    if (iter == std::numeric_limits<size_t>::max()) {
      // Visible face traversal marks visited faces with iteration counter (to mark that the face
      // has been visited on this iteration) and the max value represents unvisited faces. At this
      // point we have to reset iteration counter. This shouldn't be an issue on 64 bit machines.
      iter = 0;
    }
    ...
    // Find out the faces that have our active point on their positive side (these are the "visible
    // faces"). The face on top of the stack of course is one of them. At the same time, we create a
    ...
    while (m_possiblyVisibleFaces.size()) {
      ...
      // Store which half edge is the horizon edge. The other half edges of the face will not be
      // part of the final mesh so their data slots can by recycled.
    ...
    // Order horizon edges so that they form a loop. This may fail due to numerical instability in
    // which case we give up trying to solve horizon edge for this point and accept a minor
    ...
    // Except for the horizon edges, all half edges of the visible faces can be marked as disabled.
    // Their data slots will be reused. The faces will be disabled as well, but we need to remember
    // the points that were on the positive side of them - therefore we save pointers to them.
    ...
    for (auto faceIndex : m_visibleFaces) {
      ...
      // Disable the face, but retain pointer to the points that were on the positive side of it. We
      // need to assign those points to the new faces we create shortly.
...
void QuickHull<T>::setupInitialTetrahedron() {
  ...
  if (maxD == m_epsilonSquared) {
    // It appears that the point cloud belongs to a 1 dimensional subspace of R^3: convex hull has
    // no volume => return a thin triangle Pick any point other than selectedPoints.first and
    ...
    it = std::find_if(m_vertexData.begin(), m_vertexData.end(), [&](const vec3& ve) {
      return ve != m_vertexData[selectedPoints.first] &&
             ve != m_vertexData[selectedPoints.second] && ve != m_vertexData[thirdPoint];
    });
  ...
  // Next step is to find the 4th vertex of the tetrahedron. We naturally choose the point farthest
  ...
  if (maxD == m_epsilon) {
    // All the points seem to lie on a 2D subspace of R^3. How to handle this? Well, let's add one
    // extra point to the point cloud so that the convex hull will have volume.
    m_planar = true;
    const vec3 N1 = mathutils::getTriangleNormal(baseTriangleVertices[1], baseTriangleVertices[2],
                                                 baseTriangleVertices[0]);
    m_planarPointCloudTemp.clear();
    m_planarPointCloudTemp.insert(m_planarPointCloudTemp.begin(), m_vertexData.begin(),
                                  m_vertexData.end());
  ...
  // Enforce CCW orientation (if user prefers clockwise orientation, swap two vertices in each
  // triangle when final mesh is created)
  ...
  // Finally we assign a face for each vertex outside the tetrahedron (vertices inside the
  // tetrahedron have no role anymore)
  for (size_t i = 0; i < vCount; i++) {
