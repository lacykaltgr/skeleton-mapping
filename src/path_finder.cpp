...
double SkeletonFinder::rayFlyingValidLen(Eigen::Vector3d pos1, Eigen::Vector3d pos2, double base_radius, double min_hit_ratio) {
  ...
  for (int i = 0; i < num_steps; i++) {
    ...
    if (i > 0) {
      double hit_ratio = checkHitRatio(check_pos, forward, 3.0);
...
double SkeletonFinder::rayFlyingValidLenInverse(Eigen::Vector3d pos1, Eigen::Vector3d pos2, double base_radius, double min_hit_ratio) {
  double length = (pos2 - pos1).norm();
  double step_length = _resolution;
...
double SkeletonFinder::checkHitRatio(Eigen::Vector3d base_pos, Eigen::Vector3d forward_dir, double cut_off_length = 3.0) {
  Eigen::Vector3d downwards(0, 0, -1);
  double phi = 2 * M_PI; 
  double x, y, z, theta;
